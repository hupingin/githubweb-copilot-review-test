...
void fillRGBA8Buffer(uint8_t* img, int width, int height, int stride, int r, int g, int b) {
    ...
    ALOGI("fillRGBA8Buffer: resolution: %dX%d, stride:%d\n", width, height, stride);
    
    for (int y = 0; y < 720; y++) {
        for (int x = 0; x < 1280; x++) {
            uint8_t* pixel = img + (4 * (y*1280 + x));
            pixel[0] = r;
            pixel[1] = g;
            pixel[2] = b;
            pixel[3] = 0;
...
int drawNativeSurface(sp<NativeSurfaceWrapper> nativeSurface) {
    ...
    ALOGI("drawNativeSurface 2\n");
    // 2. set the ANativeWindow dimensions
    // err = native_window_set_buffers_user_dimensions(nativeWindow, 1200, 800);
        ...
        /*if (isFirstFrame) {
            ...
            for (int y = 0; y < nativeSurface->height(); y++) {
                for (int x = 0; x < nativeSurface->width(); x++) {
                    uint8_t* pixel = img + (4 * (y*buf->getStride() + x));
                    pixel[0] = 0;
                    pixel[1] = 0;
                    pixel[2] = 0;
                    pixel[3] = 0;
